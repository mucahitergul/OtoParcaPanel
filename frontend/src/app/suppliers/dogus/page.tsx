...
export default function DogusSupplierPage() {
  ...
  const updatePrices = async () => {
    try {
      ...
      if (updating) {
        toast.success('Tüm Doğuş fiyatları başarıyla güncellendi');
      } else {
        ...
        if (result.success && result.data && result.data.success) {
          ...
          setProducts(prev => prev.map(p => 
            p.id === product.id 
              ? { 
                  ...p, 
                  dogus_price: scrapedData.price, 
                  dogus_stock: scrapedData.stock, 
                  dogus_last_updated: new Date().toISOString() 
                }
              : p
          ));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Doğuş Tedarikçi</h1>
              <p className="text-gray-600 dark:text-gray-400 mt-2">
                Doğuş firmasından fiyat ve stok bilgilerini yönetin
              </p>
            </div>
          </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Doğuş Fiyat</TableHead>
                      <TableHead>Doğuş Stok</TableHead>
                  ...
                  <TableBody>
                    {filteredProducts.map((product) => (
                      <TableRow 
                        key={product.id}
                        className={currentUpdatingProduct === product.stok_kodu ? 'bg-purple-50 dark:bg-purple-900/20' : ''}
                      >
                        ...
                        <TableCell className="text-sm text-gray-500">
                          {formatDate(product.dogus_last_updated)}
                        </TableCell>
                        <TableCell className="text-sm text-gray-500">
